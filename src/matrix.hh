...
template <class T>
inline D1Array<T>::D1Array(uint32_t n, bool)
	: _n(n)
{
	_data = new T[n];
}
...
template <>
inline D1Array<std::vector<uint32_t> *>::D1Array(uint32_t n, bool zero)
	: _n(n)
{
...
template <>
inline D1Array<std::vector<Rating> *>::D1Array(uint32_t n, bool zero)
	: _n(n)
{
...
template <>
inline D1Array<double>::D1Array(uint32_t n, bool zero)
	: _n(n)
{
	_data = new double[n];
	if (zero)
...
template <>
inline D1Array<uint32_t>::D1Array(uint32_t n, bool zero)
	: _n(n)
{
...
template <>
inline D1Array<uint64_t>::D1Array(uint32_t n, bool zero)
	: _n(n)
{
...
template <class T>
inline D1Array<T>::D1Array(const D1Array<T> &a)
{
	copy_from(a);
}

template <class T>
inline D1Array<T>::~D1Array()
{
	delete[] _data;
}
...
template <class T>
inline D2Array<T>::D2Array(uint32_t m, uint32_t n, bool zero) : _m(m), _n(n)
{
	_data = new T *[m];
...
template <class T>
inline D2Array<T>::D2Array(const D2Array<T> &a)
{
	copy_from(a);
}
					  ...
					  uint32_t skiprows) const
{
	...
	while (!feof(f))
	{
		...
		printf("%s\n", line);
		if (l < skiprows)
		{ //skip header, if any (e.g., vowpal wabbit output)
			l++;
			skipped++;
			continue;
		}
...
template <class T>
inline D3Array<T>::D3Array(int m, int n, int k)
	: _m(m), _n(n), _k(k)
{
	_data = new T **[_m];
	for (uint32_t i = 0; i < _m; ++i)
