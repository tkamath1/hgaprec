...
void HGAPRec::write_lda_training_matrix()
{
	uint32_t x = 0;
...
void HGAPRec::write_chi_training_matrix(double wals_C)
{
...
void HGAPRec::load_ctr_beta_and_theta()
{
	char buf[4096];
...
void HGAPRec::load_chi_beta_and_theta()
{
	char buf[4096];
...
void HGAPRec::write_vwlda_training_matrix()
{
	uint32_t x = 0;
	FILE *f = fopen(Env::file_str("/ldatrain.tsv").c_str(), "w");
	for (uint32_t n = 0; n < _n; ++n)
...
void HGAPRec::write_nmf_training_matrix()
{
	uint32_t nrows = 0;
	for (uint32_t n = 0; n < _n; ++n)
	{
...
void HGAPRec::load_lda_beta_and_theta()
{
	char buf[4096];
...
void HGAPRec::load_vwlda_beta_and_theta()
{
	char buf[4096];
	_lda_gamma = new Matrix(_n, _k);
	_lda_beta = new Matrix(_k, _m);
...
void HGAPRec::load_nmf_beta_and_theta()
{
...
HGAPRec::compute_rmse()
{
		 ...
		 i != _test_map.end(); ++i)
	{
		...
		uint32_t m = r.second;
		yval_t v = i->second;
		double u = _env.graphchi ? prediction_score_chi(n, m) : (_env.hier ? prediction_score_hier(n, m) : prediction_score(n, m));
		s += (u - v) * (u - v);
		fprintf(outf, "%d\t%.5f\n", v, u);
	}
...
void HGAPRec::gen_ranking_for_users(bool load)
{
	if (_env.ctr)
	{
...
void HGAPRec::load_beta_and_theta()
{
	if (!_env.hier)
	{
		_beta.load();
